...
public class GoodsServiceImpl implements GoodsService {
    ...
    public int save(GoodsEntity goods) {
        ...
        //保存商品规格
        //保存商品详情页面显示的属性
        List<GoodsSpecificationEntity> goodsSpecificationEntityyList = goods.getGoodsSpecificationList();
        if (null != goodsSpecificationEntityyList && goodsSpecificationEntityyList.size() > 0) {
            for (GoodsSpecificationEntity item : goodsSpecificationEntityyList) {
                item.setGoodsId(id);
                goodsSpecificationDao.save(item);
            }
        }
        
        //保存商品详情页面显示的属性
        List<GoodsAttributeEntity> attributeEntityList = goods.getGoodsAttributeList();
    ...
    private String changeAttributeIds(List<Integer> attributeEntityList) {
    	String str="";
    	for(Integer id :attributeEntityList) {
    		str+=id+"_";
    	}
    	return str;
    }
    private String changeSpecificationIds(List<Integer> specificationList) {
    	String str="";
    	for(Integer id :specificationList) {
    		str+=id+"_";
    	}
    	return str;
    }
    ...
    public int update(GoodsEntity goods) {
        ...
        //删除详情页面显示的规格
     //like
        goodsAttributeDao.deleteBygoodsId(goods.getId());
        List<GoodsAttributeEntity> attributeEntityList = goods.getGoodsAttributeList();
        if (null != attributeEntityList && attributeEntityList.size() > 0) {
        ...
        if (null != goodsSpecificationEntityyList && goodsSpecificationEntityyList.size() > 0) {
            for (GoodsSpecificationEntity item : goodsSpecificationEntityyList) {
                ...
                if(item.getId()==null) {
                	...
                	goodsSpecificationDao.update(item);
                }
            }
        }
        //删除的规格
        List<GoodsSpecificationEntity> delgoodsSpecificationEntityyList = goods.getDeleteGoodsSpecificationList();
        if (null != delgoodsSpecificationEntityyList && delgoodsSpecificationEntityyList.size() > 0) {
            for (GoodsSpecificationEntity item : delgoodsSpecificationEntityyList) {
            	goodsSpecificationDao.delete(item.getId());
            }
        }
