...
var vm = new Vue({
    ...
    loadgoodsList: function(){
	    	  ...
	    	  Ajax.request({
				    url: "../coupongoods/selectAllCouponGoods",
                    async: true,
     ...
     refreshGoodsGrid: function(){
        	$("#goodsGrid").Grid({
			        ...
			        colModel: [
		                 ...
		                 {label: '商品', name: 'goodsName', index: 'goodsName', width: 380},
			            
			              {
			                label: '商品图片', name: 'picUrl', index: 'pic_url', width: 300, formatter: function (value) {
	    ...
	    refreshCouponGoodsGrid: function(){
        	$("#couponGoodsGrid").Grid({
			        datatype : "local",
			        colModel: [
			             {label: 'id', name: 'id', index: 'id', key: true, hidden: true},

		                 {label: '商品', name: 'goodsName', index: 'goodsName', width: 500},
	
			              {
			                label: '商品图片', name: 'picUrl', index: 'picUrl', width: 300, formatter: function (value) {
			                    return transImg(value);
			                }
			             }],
			           
	        });
	        if(vm.coupon.couponGoodsList==null)
	        	vm.coupon.couponGoodsList=[];
	        $("#couponGoodsGrid").jqGrid('clearGridData');     
	        for(var i=0;i< vm.coupon.couponGoodsList.length;i++){
	       		
	        	 $("#couponGoodsGrid").jqGrid('addRowData', i+1, vm.coupon.couponGoodsList[i]);  
	       		 
	       	}
        },
        addCouponGood: function(){
        	
        	
        	
        	vm.refreshGoodsGrid();
        	vm.showCouponGoods=true;
        	openWindow({
                title: "添加商品",
                area: ['860px', '600px'],
        ...
        deleteCouponGoods:function(){
        
        	var ids = getSelectedRows("#couponGoodsGrid");
            if (ids == null) {
                return;
            }
            confirm('确定要删除选中的记录？', function () {
            	...
            	for(var index=ids.length; index>0;index--){
            		...
            		if(vm.coupon.couponGoodsList[ids[index-1]-1].id!=null){
            		
            			vm.coupon.delCouponGoodsList.push(vm.coupon.couponGoodsList[ids[index-1]-1]);
            		}
            		
            		vm.coupon.couponGoodsList.splice(ids[index-1]-1,1);
	            }
	             alert('提交成功', function (index) {
                            vm.refreshCouponGoodsGrid();
                        });
        ...
        commitCouponGoodsList:function(){
	        var ids = getSelectedRows("#goodsGrid");
	            if (ids == null) {
	                return;
	        }
        	for(var index=ids.length; index>0;index--){
        		 var goods=vm.goodsList[ids[index-1]-1];
        		 var find=false;
        		 for(var i=0;i< vm.coupon.couponGoodsList.length;i++){
	        		if(vm.coupon.couponGoodsList[i].goodsId==goods.goodsId)
	        		{
	        		 find=true;
	        		 break;
	        		}
	       		  ...
	       		  if(find==false){
	        	 	vm.coupon.couponGoodsList.push(goods);
	        	  }
	       	}
	       	 alert('提交成功', function (index) {
                            vm.refreshCouponGoodsGrid();
                            vm.showCouponGoods=false
                             layer.closeAll();
                        });
        },
