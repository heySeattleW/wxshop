$(function () {
    ...
    $("#jqGrid").Grid({
        ...
        colModel: [
            ...
            {
                label: '取货方式', name: 'pickUpType', index: 'pick_up_type', width: 80, formatter: function (value) {
                    if (value == '0') {
                        return '快递';
                    } else if (value == '1') {
                        return '自取';
...
let vm = new Vue({
    ...
    methods: {
        ...
        refreshGoodGrid: function(){
        	$("#goodGrid").Grid({
			        datatype : "local",
			        colModel: [
			             {label: 'id', name: 'id', index: 'id', key: true, hidden: true},  
		                 {label: '商品名称', name: 'goodsName', index: 'goodsName', width: 180},
			             {label: '规格', name: 'goodsSpecifitionNameValue', index: 'goodsSpecifitionNameValue', width: 380},
			             {label: '价格', name: 'retailPrice', index: 'retailPrice', width: 380},
			              {
			                label: '规格图片', name: 'listPicUrl', index: 'listPicUrl', width: 200, formatter: function (value) {
			                    return transImg(value);
			                }
			             }]
	        });
	        if(vm.order.orderGoodsEntityList==null)
	        	vm.order.orderGoodsEntityList=[];
	        $("#goodGrid").jqGrid('clearGridData');     
	        for(var i=0;i< vm.order.orderGoodsEntityList.length;i++){
	        	 $("#goodGrid").jqGrid('addRowData', i+1, vm.order.orderGoodsEntityList[i]);  
	       	}
        },
