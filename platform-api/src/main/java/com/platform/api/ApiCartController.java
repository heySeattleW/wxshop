...
public class ApiCartController extends ApiBaseAction {
    ...
    public Object getCart(@LoginUser UserVo loginUser) {
        ...
        for (CartVo cartItem : cartList) {
            ...
            cartItem.setRetail_price(productInfo.getRetail_price());
            cartItem.setMarket_price(productInfo.getMarket_price());
            System.out.println("cartItem.getRetail_price()="+cartItem.getRetail_price());
    ...
    public Object add(@LoginUser UserVo loginUser) {
        ...
        //取得规格的信息,判断规格库存
        System.out.println("begin productId="+productId);
        ...
        if (null == cartInfo) {
            ...
            cartInfo.setGoods_specifition_name_value(productService.getGoods_specificationName(productInfo));
    ...
    public Object update(@LoginUser UserVo loginUser) {
        ...
        ProductVo productInfo = productService.queryObject(productId);
        //查找是否是特价商品
        BigDecimal activity_price=productService.queryActivityPrice(productId);
        if(activity_price!=null) {
        	productInfo.setRetail_price(activity_price);
        }
    ...
    public Object checkout(@LoginUser UserVo loginUser, Integer couponId, @RequestParam(defaultValue = "cart") String type) {
        ...
        //默认收货地址
        String addressId=this.request.getParameter("addressId");
        Map param = new HashMap();
        ...
        BigDecimal minGoodsPrice = new BigDecimal(0.00);
        List<CouponGoodsVo> couponsEntityList =null;
        if (couponId != null && couponId != 0) {
            CouponVo couponVo = apiCouponMapper.getUserCoupon(couponId);
            if (couponVo != null) {
                couponPrice = couponVo.getType_money();
                minGoodsPrice=couponVo.getMin_goods_amount();
            }
           
            couponsEntityList = goodsService.queryCouponoodsList(couponId);
        }
        // * 获取要购买的商品和总价
        ...
        if (type.equals("cart")) {
            ...
            String goodsSepcifitionValue[]=null;
            cartVo.setGoods_specifition_name_value(productService.getGoods_specificationName(productInfo));
        ...
        for(CartVo cartVo:checkedGoodsList) {
        	if(couponsEntityList!=null) {
	        	for(CouponGoodsVo couponGoodsVo:couponsEntityList) {
	        		System.out.println("cartVo.getGoods_id()="+cartVo.getGoods_id()+" "+couponGoodsVo.getGoodsId());
	        		if(cartVo.getGoods_id().equals(couponGoodsVo.getGoodsId())) {
	        			totalPrice=totalPrice.add(cartVo.getRetail_price().multiply(new BigDecimal(cartVo.getNumber())));
	        			break;
	        		}
	        	}
        	}
       ...
       System.out.println("totalPrice="+totalPrice+" minGoodsPrice="+minGoodsPrice);
        if(totalPrice.compareTo( minGoodsPrice)<0) {
        	couponPrice=new BigDecimal(0.00);
