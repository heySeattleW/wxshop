...
public class ApiOrderService {
     ...
     */
    @Transactional
    public void updateGoods_number(OrderVo order) {
    	...
    	for(OrderGoodsVo orderGoodsVo:orderGoods)
    	{
    		...
    		productVo.setGoods_number(productVo.getGoods_number()-orderGoodsVo.getNumber());
    		productService.update(productVo);
    		InfoVo infoVo=new InfoVo();
    		infoVo.setCreateTime(new Date());
    		infoVo.setUserId(order.getUser_id());
    		UserVo userVo=userDao.queryObject(order.getUser_id());
    		infoVo.setUserName(userVo.getNickname());
    		String info=orderGoodsVo.getGoods_name();
    		infoVo.setInfo(info);
    		apiInfoMapper.save(infoVo);
    	}
    }
    @Transactional
    public Map<String, Object> submit(JSONObject jsonParam, UserVo loginUser) {
        ...
        Integer pickUpType = jsonParam.getInteger("isDdzt");
...
//        AddressVo addressVo = jsonParam.getObject("checkedAddress",AddressVo.class);
        ...
        orderInfo.setAddress(addressVo.getDetailInfo()+addressVo.getTownName());
        ...
        orderInfo.setPickUpType(pickUpType);
        //开启事务，插入订单信息和订单商品
