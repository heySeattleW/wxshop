...
public class SysSmsLogServiceImpl implements SysSmsLogService {
    ...
    public SysSmsLogEntity sendSms(SysSmsLogEntity smsLog) {
        ...
        save(smsLog);
        */
    	String flag=SmsUtil.sendAlyunSms(smsLog.getMobile(),smsLog.getContent(), DateUtils.format(smsLog.getStime(), "yyyy-MM-dd HH:mm:ss"), smsLog.getExtno());
    	if ("0".equals(flag)) {
    		
        } else {
           
        }
    	smsLog.setSendStatus(Integer.parseInt(flag));
    	save(smsLog);
    	return smsLog;
    }
}
